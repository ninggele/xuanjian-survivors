...
function card(run,id,i,preview,rewards,rewardDesc){const training=trainingInfo(run,id);if(run.choiceKind!=='gift'&&training){const lv=trainingRank(run,id);return `<article class="card decision-card training-card"><small>修行强化 · ${lv} → ${lv+1} 阶 / ${training.max}</small><strong>${training.title}</strong><p class="decision-purpose">${training.purpose}</p><span class="decision-cost">被动 · 不耗法力 · 本局有效</span><p class="decision-gain">${training.stat(lv)} → ${training.stat(lv+1)}</p>${id==='vigor'?'<span class="decision-condition">同时回复 15 气血，上限即止</span>':id==='recovery'?'<span class="decision-condition">不恢复寿元，不救致死</span>':''}<span class="decision-relation">${run.item==='screen'&&id==='reach'?'持屏仅拾取受益':training.fit}</span><details class="decision-details"><summary>数值与机制</summary><p>${training.detail}</p><p>本作局内修行规则，不是原著神通，不占神通槽。最多 ${training.max} 阶，重开清空。</p></details><button class="choose-card" data-choice="${i}">${i+1} · ${lv?'精修一阶':'修习强化'}</button></article>`;}const foundation=SKILLS.find(s=>s.id===id);if(run.choiceKind!=='gift'&&foundation){const lv=run.lv(id),formed=lv===2,opening=run.choiceKind==='opening',foundationBrief=id==='gate'?'落门短暂镇敌 · 不召兵':'蕴养中 · 尚无神通战斗效果';return `<article class="card decision-card ${opening?'foundation-card':''} ${formed?'manifesting-card':''}">${root.XJUIArt?.skill(id,foundation.dao)||''}<small>${foundation.dao} · ${run.skillStage(id)} → ${formed?'神通':'仙基'+(lv+1)+'重'}</small><strong>${foundation.name}</strong><p class="decision-purpose">${formed?copy[id]?.[0]||foundation.desc:foundationBrief}</p>${formed?'':`<p class="foundation-future"><span>神通成就后</span>${copy[id]?.[0]||foundation.desc}</p>`}${opening?'':`<span class="decision-cost">${formed?(['body','gold'].includes(id)?'被动 · 不耗法力':'成就后默认自动 · '+X.Rules.costs[id]+' 法力'):'蕴养仙基 · 以器物普攻杀敌'}</span>`}<details class="decision-details"><summary>修行说明</summary><p>${run.foundationDescription(id)}</p>${formed?`<p>${preview(id,2).split(' →<br>').pop()}</p>`:''}</details><button class="choose-card" data-choice="${i}" ${run.canCultivate(id)?'':'disabled'}>${i===9?'0':i+1} · ${formed?'成就神通':lv?'蕴养二重':'选择仙基'}</button></article>`;}const gift=run.choiceKind==='gift',s=(gift?GIFTS:SKILLS).find(s=>s.id===id),lv=run.lv(id),passive=['body','gold'].includes(id),info=gift?gifts[id]:copy[id],cost=!gift&&s&&!passive?`${run.manualSkill===id?(run.hasFruitActive()?'F 蓄能三次敕令 · 不耗法力':'F 手动 · '+run.skillCost(id)+' 法力'):'自动施放 · '+run.skillCost(id)+' 法力'} · 与其他神通共用法力`:gift?gifts[id][1]:s?'被动 · 不耗法力':'立即生效';
...
function casting(run){return SKILLS.filter(s=>run.lv(s.id)).map(s=>`<div class="casting-row"><b>${s.name} · ${run.skillStage(s.id)}</b><span>${run.lv(s.id)===3?root.XJFeedback.state(run,s.id):run.foundationDescription(s.id)}</span></div>`).join('')||'<p>尚未修习</p>';}
...
function growth(run,id,oldFruits){const training=trainingInfo(run,id);if(training)return training.title+' · '+training.stat(trainingRank(run,id))+(id==='vigor'?' · 气血已回复':'');if(copy[id])return name(id)+' · '+run.skillStage(id)+' · '+(run.lv(id)===3?copy[id][0]:run.foundationDescription(id));if(gifts[id])return (GIFTS.find(g=>g.id===id).name)+' · '+gifts[id][1];return '';
}
...
function compactRoutes(run){return `<div class="growth-status"><b>${run.realm} · ${run.masteredCount()}/5 神通</b><span>${run.masteredCount()===5?'五法圆满':'下一道神通需修为 '+run.breakthroughNeed()+' 级'}</span></div><div class="growth-five">${run.pathSkills().map(s=>`<span title="${run.skillStage(s.id)}">${s.name}<i>${'●'.repeat(run.lv(s.id))}${'○'.repeat(3-run.lv(s.id))}</i></span>`).join('')}</div>`;}
const api={compactRoutes,loopPanel,gradeBadge,trainingInfo,trainingSummary,copy,weapons,gifts,name,benefit,fit,relation,routes,card,casting,loadout,instruction,growth,attempt,itemState,actionClass};root.XJExperience=api;if(typeof module!=='undefined')module.exports=api;
