...
function mana(run){const count=SKILLS.filter(s=>run.lv(s.id)&&run.skillCost(s.id)&&!(run.item==='screen'&&s.id==='spring')).length;return `<p class="mana-context">当前 ${count} 道主动神通共用法力 · 每秒回复 ${run.regen.toFixed(1)} · 暂停可指定一道主修优先施放</p>`;}
function primary(run){return `<label class="manual-picker primary-picker">主修优先 <select id="primarySelect"><option value="">均衡调度</option>${SKILLS.filter(s=>run.canPrimary(s.id)).map(s=>`<option value="${s.id}" ${s.id===run.primarySkill?'selected':''}>${s.grade}级 · ${s.name}</option>`).join('')}</select></label><p class="primary-help">主修就绪时优先蓄法；冷却或无目标时让行，紧急防御仍优先。其他神通缺法先让行，等待一秒后轮候蓄法。</p>`;}
