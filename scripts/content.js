...
if(id==='light'){const r=195+lv*25;this.lightPulse(p.x,p.y,r,lv,false);this.allies.forEach(a=>{a.empowered=5;a.empoweredAt=this.t;a.cd=0;});this.cool.light=8;if(this.fruit)this.zones.forEach(z=>{if(z.kind==='edict')z.life=Math.max(z.life,z.age+5);});}
...
if(id==='rain'){nearest=(lv===3?this.zones.list().find(z=>z.kind==='peril'&&dist(z,p)<500):null)||this.cluster(500,110+lv*20)||nearest;const fruitRain=!!this.fruits.lushui;this.zones.add({kind:'rain',x:nearest.x,y:nearest.y,r:(110+lv*20)*(fruitRain?1.25:1),life:(4+lv*.5)*(fruitRain?1.35:1),age:0,tick:0,lv,fruitRain});this.cool.rain=9;}
