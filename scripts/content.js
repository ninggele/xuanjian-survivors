...
// Chapter 六合: prohibition and dispelling. Triple cadence is a game adaptation.
Run.prototype.lightPulse=function(x,y,r,lv,fruit=false){const stun=1.2+lv*.4;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,{x,y})<r+e.r)e.lightUntil=this.t+stun;});this.area(x,y,r,14+lv*8,stun,true);this.bullets.forEach(b=>{if(b.enemy&&dist(b,{x,y})<r)this.bullets.remove(b);});this.allies.forEach(a=>{a.empowered=5;a.empoweredAt=this.t;a.cd=0;});this.effect(fruit?'sixfoldEdict':'light',x,y,r,fruit?.38:.7);if(fruit)this.telemetry.edictPulses=(this.telemetry.edictPulses||0)+1;};
Run.prototype.updateSkills=function(dt,nearest,manual=false){const p=this.p;
if(!manual&&this.lv('body')===3){let count=0,weight=0;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,p)<170){count++;weight+=e.boss?3:e.elite?2:1;}});p.pressure=count>=2?Math.min(1,weight/5):0;p.danger=p.pressure;
...
if(id==='spring'){this.springCharges=4+(lv===3?2:0);this.cool.springPower=8;this.cool.spring=7;this.effect('spring',p.x,p.y,65,.6);}
...
if(id==='conceal'){this.lastSeen={x:p.x,y:p.y};this.cool.hidden=1.5+lv*.5;this.cool.conceal=10;this.effect('conceal',p.x,p.y,65,1.5+lv*.5);}
...
if(id==='gate'&&lv<3){const target=this.cluster(430,55+lv*20)||nearest,r=55+lv*20;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,target)<r){e.stun=Math.max(e.stun,(.4+lv*.2)*(e.boss?.35:e.elite?.6:1));e.phase='move';e.cd=Math.max(e.cd,.4);}});this.effect('foundationGate',target.x,target.y,r,.7);this.cool.gate=9;continue;}
if(id==='gate'){let count=lv+1+(lv===3?2:0);for(let i=0;i<count;i++)this.allies.add({x:p.x+Math.cos(i*2.4)*55,y:p.y+Math.sin(i*2.4)*55,life:7+lv,cd:i*.12,empowered:0});this.effect('gate',p.x,p.y,80,.8);this.cool.gate=9;}
...
if(id==='light'){const r=195+lv*25;this.lightPulse(p.x,p.y,r,lv,false);this.cool.light=8;}
...
if(id==='rain'){nearest=(lv===3?this.zones.list().find(z=>z.kind==='peril'&&dist(z,p)<500):null)||this.cluster(500,110+lv*20)||nearest;const fruitRain=lv===3;this.zones.add({kind:'rain',x:nearest.x,y:nearest.y,r:(110+lv*20)*(fruitRain?1.25:1),life:(4+lv*.5)*(fruitRain?1.35:1),age:0,tick:0,lv,fruitRain});this.cool.rain=9;}
...
Run.prototype.updateZones=function(dt){if(this.lightEdict){const f=this.lightEdict;f.age+=dt;while(f.next<=2&&f.age+1e-9>=f.next*.6){this.lightPulse(f.x,f.y,f.r,f.lv,true);f.next++;}if(f.next>2)this.lightEdict=null;}this.zones.forEach(z=>{if(z.kind==='hazard')return;
if(z.kind==='zhiming'){z.x=this.p.x;z.y=this.p.y;if(z.tick<=0){z.tick=1;const casters=[3,4,8,9,10],targets=this.enemies.list().filter(e=>e.born<=0&&dist(e,z)<280).sort((a,b)=>Number(casters.includes(b.type))-Number(casters.includes(a.type))||dist(a,z)-dist(b,z));const e=targets[0];if(e){this.hit(e,18+z.lv*6);if(e.active&&casters.includes(e.type)){const duration=(.4+z.lv*.2)*(e.elite?.5:1);e.zhimingUntil=Math.max(e.zhimingUntil||0,this.t+duration);e.phase='move';e.timer=0;e.wallPoints=null;e.cd=Math.max(e.cd,duration);this.effect('zhimingSeal',e.x,e.y,18,duration);}this.effect('zhimingStrike',e.x,e.y,22,.28);}}}
if(z.kind==='dali'){z.x=this.p.x;z.y=this.p.y;if(z.tick<=0){z.tick=.5;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,z)<z.r)this.hit(e,9+z.lv*4);});}}
...
if(z.kind==='edict'){this.enemies.forEach(e=>{if(dist(e,z)<z.r)e.mark=.25;});this.allies.forEach(a=>{if(dist(a,z)>z.r+40){a.x+=(z.x-a.x)*dt*2;a.y+=(z.y-a.y)*dt*2;}});}
...
if(z.kind==='rain'&&z.tick<=0){z.tick=.55;this.enemies.forEach(e=>{if(dist(e,z)<z.r){if(z.age>=.8)e.exposed=.8;this.hit(e,(6+z.lv*3)*(z.fruitRain?1.5:1));}});}
...
// Union of living rain fields: overlapping fields never accelerate the two-second requirement.
const rains=this.zones.list().filter(z=>z.kind==='rain'&&z.fruitRain&&z.age<z.life);
this.enemies.forEach(e=>{
 e.rainBroken=Math.max(0,(e.rainBroken||0)-dt);
 const wet=e.born<=0&&rains.some(z=>dist(e,z)<z.r);
 e.rainSoak=wet?Math.min(2,(e.rainSoak||0)+dt):0;
 e.rainPressure=e.shield?e.rainSoak/2:0;
 if(e.shield&&e.rainSoak>=2){if(!(e.rainBroken>0)){this.effect('rainBreak',e.x,e.y,28,.65);this.telemetry.rainBreaks=(this.telemetry.rainBreaks||0)+1;}e.rainBroken=3;}
});baseZones.call(this,dt);};
Run.prototype.updateAllies=function(dt){this.allies.forEach(a=>{a.life-=dt;a.empowered=Math.max(0,a.empowered-dt);a.cd-=dt;if(a.life<=0){this.allies.remove(a);return;}let e=this.nearest(a,410);if(e){let d=dist(a,e)||1;if(d>30){a.x+=(e.x-a.x)/d*225*dt;a.y+=(e.y-a.y)/d*225*dt;}else if(a.cd<=0){a.attackAt=this.t;a.aim=Math.atan2(e.y-a.y,e.x-a.x);a.cd=a.empowered>0?.45:.8;this.hit(e,(12+this.lv('gate')*6)*(a.empowered>0?1.6:1),a.empowered>0);this.effect('allyHit',e.x,e.y,25,.22,{angle:a.aim});}}else{a.x+=(this.p.x-a.x)*dt*2;a.y+=(this.p.y-a.y)*dt*2;}});};
