...
function state(run,id){if(run.item==='screen'&&id==='spring')return '持屏停用 · 仅计果位';if(run.item==='screen'&&id==='gold')return '持屏无普攻 · 不生效';const p=run.p,z=run.zones.list().find(z=>z.kind===id),c=run.cool;
