...
function figure(x,y,kind,phase,walk,dir=1,casting=0,pose={}){c.save();c.translate(x,y);c.scale(.80,.80);ellipse(1,3,16,4,'#474b402d');ellipse(0,2,10,2.4,'#303e355e');const gait=pose.gait??phase*10,amount=pose.walkAmount??(walk?1:0),weight=Math.sin(gait)*amount;
const bob=-Math.abs(Math.sin(gait))*.45*amount;
c.scale(dir,1);
// A narrow porcelain rim identifies the player in crowds; no glow or extra hit area.
if(pose.hero){c.beginPath();c.moveTo(-4,-35);c.lineTo(-12,-32);c.lineTo(-9,-22);c.lineTo(-15,-5);c.lineTo(-2,-8);c.lineTo(1,-13);c.lineTo(5,-6);c.lineTo(14,-7);c.lineTo(9,-23);c.lineTo(10,-32);c.lineTo(4,-36);c.strokeStyle='#f7faf3';c.lineWidth=3;c.lineJoin='round';c.stroke();ellipse(0,-46,9,11,'#f7faf3');}
const kick=(pose.recoil||0)/.18;
...
if(pose.armor){
 ...
 const hem=Math.sin(gait-.7)*1.8*amount;
 c.beginPath();c.moveTo(-5,-36);c.lineTo(-10,-32);c.lineTo(-8,-22);c.lineTo(-14+hem,-5);c.lineTo(-2+hem*.3,-8);c.lineTo(1,-14);c.lineTo(5+hem*.3,-6);c.lineTo(13+hem,-7);c.lineTo(8,-23);c.lineTo(9,-32);c.lineTo(4,-36);c.closePath();c.fillStyle=pose.hero?'#f1eadb':'#b7d0c4';c.fill();c.strokeStyle='#244c48';c.lineWidth=1.5;c.stroke();
 c.beginPath();c.moveTo(-4,-35);c.lineTo(3,-28);c.lineTo(5,-23);c.lineTo(-2,-23);c.lineTo(-7,-31);c.closePath();c.fillStyle=pose.hero?'#faf6e9':'#e6efe7';c.fill();
 c.beginPath();c.moveTo(4,-25);c.lineTo(8,-23);c.lineTo(13+hem,-7);c.lineTo(7+hem*.3,-8);c.lineTo(2,-17);c.closePath();c.fillStyle='#183a38';c.fill();
 // One asymmetric outer robe. The edge follows the existing animated hem.
 c.beginPath();c.moveTo(-6,-36);c.lineTo(-12,-32);c.lineTo(-9,-25);c.lineTo(-1,-18);c.lineTo(3+hem*.3,-6);c.lineTo(13+hem,-7);c.lineTo(8,-23);c.lineTo(-1,-28);c.closePath();c.fillStyle=pose.hero?'#163f40':'#285b57';c.fill();c.strokeStyle='#1e4341';c.lineWidth=1;c.stroke();
 line([[-6,-35],[-1,-29],[6,-24]],'#7aa99a',1.25);line([[4,-19],[10+hem,-8]],'#356e63',1.3);
 line([[-5,-35],[3,-27],[7,-33]],'#dde3ce',1.3);
 line([[-11+hem,-7],[-3+hem*.3,-10]],'#c5c49b',1.1);line([[6+hem*.3,-8],[11+hem,-9]],'#c5c49b',1.1);line([[4,-35],[-4,-26],[-7,-12]],'#739789',.9);
 line([[-8,-23],[8,-23]],'#3b5c51',3);ellipse(1,-23,2.1,1.8,'#baa875');ellipse(1,-23,.8,.8,'#517c6d');line([[5,-22],[7+hem,-14],[5+hem,-9]],'#b1a06d',1.8);
 if(!pose.hero)line([[-11+hem,-7],[-7,-20]],'#41695e',1);line([[7,-20],[11+hem,-8]],'#142f30',1);if(!pose.hero){line([[-3,-19],[-6+hem*.3,-9]],'#95b1a3',.9);line([[5,-18],[8+hem*.3,-10]],'#95b1a3',.9);}
}
...
function arm(shoulder,hand,bend,col=ink){
 ...
 line([shoulder,elbow,joint],pose.armor?'#887d52':'#284b49',7.1);line([shoulder,elbow,joint],pose.armor?'#d9dccb':pose.hero?'#ede5d5':'#52786c',4.8);
 if(pose.hero){line([shoulder,[shoulder[0]+(elbow[0]-shoulder[0])*.7,shoulder[1]+(elbow[1]-shoulder[1])*.7]],'#214747',5.8);line([[elbow[0]-1,elbow[1]-1],[joint[0]-1,joint[1]-1]],'#fff6e5',1.2);}line([joint,hand],col,2);
...
if(kind===3){
 ...
 front=point(8,-2);back=point(-8,8);arm([-3,-32],back,1,'#65736b');arm([4,-32],front,-1);
 if(kind===5){c.beginPath();[point(3,-12),point(12,-15),point(17,-11),point(17,6),point(3,6)].forEach((v,i)=>i?c.lineTo(...v):c.moveTo(...v));c.closePath();c.fillStyle='#e3e7e5';c.fill();c.strokeStyle='#626c80';c.lineWidth=1.5;c.stroke();line([point(9,-10),point(7,-4),point(12,-4),point(9,2)],'#8b819e',1.3);}
 else for(let i=0;i<8;i++){const x=-2+i*2.8,dy=i%2?2:0;c.beginPath();[point(x,-12+dy),point(x+2.8,-10-dy),point(x+2.8,9-dy),point(x,7+dy)].forEach((v,j)=>j?c.lineTo(...v):c.moveTo(...v));c.closePath();c.fillStyle=i%2?'#a6b6a8':'#d9dfcc';c.fill();c.strokeStyle='#657665';c.lineWidth=.7;c.stroke();}
}else{
 ...
 arm([-3,-32],back,1,'#65736b');arm([4,-32],front,-1);
 c.beginPath();[point(4,-1.6,angle),point(31,-1.4,angle),point(38,0,angle),point(31,1.5,angle),point(4,1.8,angle)].forEach((p,i)=>i?c.lineTo(...p):c.moveTo(...p));c.closePath();c.fillStyle='#a7b8be';c.fill();c.strokeStyle=stroke;c.lineWidth=.9;c.stroke();line([point(6,0,angle),point(35,0,angle)],'#fff5df',1);line([point(6,1.5,angle),point(31,1.3,angle)],'#5f747c',.65);line([point(-5,0,angle),point(3,0,angle)],'#796c47',2.8);
...
if(pose.robe){
 ellipse(0,-46,8,10,'#e8e9d8');ellipse(0,-46,8,10,'#3e514b',true,1.6);
